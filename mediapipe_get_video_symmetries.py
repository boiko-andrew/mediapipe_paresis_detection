...
def get_mediapipe_video_symmetries(video_full_file_name, markup_full_file_name, output_images_file_path):
    ...
    while read_result:
        ...
        if frame_num in markup['peak_frame'].to_list():
            ...
            image_with_points = put_image_points(frame, frame_landmarks, REGULAR_RADIUS, LARGE_RADIUS,
                                                 WHITE_COLOUR, RED_COLOUR)
            ...
            eyebrows_symmetries.append(eyebrows_symmetry)
            eyes_symmetries.append(eyes_symmetry)
            nose_symmetries.append(nose_symmetry)
            mouth_symmetries.append(mouth_symmetry)
    ...
    markup.insert(5, 'eyebrows_symmetries', pd.Series(eyebrows_symmetries))
    markup.insert(6, 'eyes_symmetries', pd.Series(eyes_symmetries))
    markup.insert(7, 'nose_symmetries', pd.Series(nose_symmetries))
    markup.insert(8, 'mouth_symmetries', pd.Series(mouth_symmetries))
